...
let timelineActiveHighlightNode = null;

const getTimelineMessageKey = (entry, index) =>
  entry?.groupKey ||
  entry?.turnKey ||
  entry?.anchorMessageKey ||
  `timeline-turn-${index}`;

const resolveTurnAnchorNode = (turn) => {
  if (!turn) {
    return null;
  }
  const anchorMessage =
    turn.anchorMessageKey && typeof getMessageByKey === "function"
      ? getMessageByKey(turn.anchorMessageKey)
      : null;
  if (anchorMessage?.node instanceof HTMLElement && anchorMessage.node.isConnected) {
    return anchorMessage.node;
  }

  if (Array.isArray(turn.messageKeys) && typeof getMessageByKey === "function") {
    for (const key of turn.messageKeys) {
  ...
  return turn.anchorNode instanceof HTMLElement ? turn.anchorNode : null;
};

const getGroupUserMessage = (group) => {
  if (!group || typeof getMessageByKey !== "function" || !group.userMessageKey) {
    return null;
  }
  return getMessageByKey(group.userMessageKey);
};
const getGroupAssistantMessage = (group) => {
  if (!group || typeof getMessageByKey !== "function") {
    return null;
  }
...
const getTimelineSourceNodes = () => {
  ...
  const visibleGroups = typeof getVisibleGroups === "function" ? getVisibleGroups() : [];
  const seen = new Set();
  const uniqueGroups = [];
  visibleGroups.forEach((group, index) => {
    ...
    const anchorNode = group.anchorNode instanceof HTMLElement ? group.anchorNode : null;
    if (!(anchorNode instanceof HTMLElement) || !anchorNode.isConnected) {
      return;
    }
    ...
    uniqueGroups.push({
      ...group,
      anchorNode,
      key: getTimelineMessageKey(group, index),
    });
  });
  return uniqueGroups;
};
...
const getConfiguredTimelineMaxNodes = () => {
  const settingsValue = Number(state?.settings?.timelineMaxNodes);
  if (Number.isFinite(settingsValue)) {
    return Math.max(1, Math.round(settingsValue));
  }

  const legacyValue = Number(state?.timelineMaxNodes);
  if (Number.isFinite(legacyValue)) {
    return Math.max(1, Math.round(legacyValue));
  }

  if (Number.isFinite(TIMELINE_MAX_NODES)) {
    return Math.max(1, Math.round(TIMELINE_MAX_NODES));
  }
...
const buildTimelineSourceSignature = (messages) =>
  messages.map((message, index) => getTimelineMessageKey(message, index)).join("|");

const isSameTimelineSource = (messages, signature) =>
  timelineState.sourceSignature === signature &&
  timelineState.sourceNodes.length === messages.length &&
  messages.every(
    (message, index) =>
      getTimelineMessageKey(message, index) ===
      getTimelineMessageKey(timelineState.sourceNodes[index], index)
  );
...
const highlightTimelineMessageNode = (node) => {
  ...
  if (timelineActiveHighlightNode instanceof HTMLElement) {
    timelineActiveHighlightNode.classList.remove("chatgpt-toolkit-timeline-target");
  }

  timelineActiveHighlightNode = node;
  ...
  timelineHighlightTimer = setTimeout(() => {
    ...
    timelineActiveHighlightNode = null;
    timelineHighlightTimer = null;
  }, 1400);
};
...
const buildTimelineItemsFromSourceNodes = (groups) => {
  const withTimestamps = [];
  let previousTimestamp = null;
  groups.forEach((group, index) => {
    const node = group?.anchorNode instanceof HTMLElement ? group.anchorNode : null;
    if (!(node instanceof HTMLElement)) {
      return;
    }
    ...
    if (qText && aText) {
      ...
      previewSeed = extractMessageText(node);
    }
    const order = Number.isFinite(group?.index)
      ? group.index
      : index + 1;
    ...
    withTimestamps.push({
      ...
      timestamp,
      order,
      previewText: normalizeTimelineText(previewSeed),
    });
  });
  ...
  const limitedItems = limitTimelineItems(sortedItems, getConfiguredTimelineMaxNodes());
  return {
...
const handleTimelineWheel = (event) => {
  ...
  // Prevent default page scroll only when timeline has scrollable content.
  if (maxScrollTop > 1) {
    event.preventDefault();
...
const renderTimeline = () => {
  ...
  timelineState.refreshPending = false;
  // Mark rendering phase to avoid MutationObserver loops.
